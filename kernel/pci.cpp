...
namespace{
  ...
  MSICapability ReadMSICapability(const Device &dev, uint8_t cap_addr)
  {
    ...
    if(msi_cap.header.bits.addr_64_capable){
      msi_cap.msg_upper_addr = ReadConfReg(dev, msg_data_addr);
  ...
  Error ConfigureMSIRegister(const Device &dev, uint8_t cap_addr, uint32_t msg_addr, uint32_t msg_data, unsigned num_vector_exponent)
  {
  ...
  Error ConfigureMSIXRegister(const Device &dev, uint8_t cap_addr, uint32_t msg_addr, uint32_t msg_data, unsigned num_vector_exponent)
  {
...
namespace pci{
  ...
  {
    auto shl = [](uint32_t x, unsigned bits){
      return x << bits;
    };
  ...
  Error ScanAllBus() 
  {
    ...
    for(uint8_t function = 1; function != 8; ++function){   // when multi host-bridges
      if(ReadVendorId(0,0,function) == 0xFFFFU){  // empty
        continue;
      }
  ...
  // enable MSI interruption of specified device

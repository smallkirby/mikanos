...
void LayerManager::Move(unsigned id, Vector2D<int> new_position) 
{
  FindLayer(id)->Move(new_position);
}

void LayerManager::MoveRelative(unsigned id, Vector2D<int> pos_diff) 
{
  FindLayer(id)->MoveRelative(pos_diff);
}

void LayerManager::UpDown(unsigned id, int new_height) 
{
  if(new_height < 0){
    Hide(id);
    return;
  ...
  // if the layer was not visible, just insert it onto stack.
...
void LayerManager::Hide(unsigned id) 
{
  auto layer = FindLayer(id);
...
Layer* LayerManager::FindLayer(unsigned id) 
{
